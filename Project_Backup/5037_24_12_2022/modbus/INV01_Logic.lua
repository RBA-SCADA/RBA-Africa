...
function logCsv(file, msg)
 ...
 if file1 == nil then
  ...
  file1:write(anlagen_id..",SN:DG_LOG_IM01_,DG_LOG,0.0.0.0,2".."\n")
 ...
 if file2 == nil then
  ...
  file2:write(anlagen_id..",SN:ZE_LOG_IM01_,ZE_LOG,0.0.0.0,3".."\n")
 ...
 if ampZEFUHF == nil then
  ...
  function(dev)
   ...
   for devV in pairs(dgCtlDev) do  
     ...
     if  ((pacramp ~= 2) or (pactimeout ~= 90) or (cmdEnable ~= 1) or (pacwintime ~= 2)) then
      if WR.isOnline(devV) then
       WR.writeHexOpts(devV, cmdEnableWrite, bit.tohex(1,4),0x6)
       WR.writeHexOpts(devV, pacrampWrite, bit.tohex(2,4),0x6)     
       WR.writeHexOpts(devV, pactimeoutWrite, bit.tohex(90,4),0x6)
       WR.writeHexOpts(devV, pacwintimeWrite, bit.tohex(2,4),0x6)
      end
     end
   end
   --]]
   ...
   local inv6_Com = WR.read("SN:INV13", "COMMUNICATION_STATUS")
   local inv7_Com = WR.read("SN:INV14", "COMMUNICATION_STATUS")
   local inv8_Com = WR.read("SN:INV15", "COMMUNICATION_STATUS")
   local inv9_Com = WR.read("SN:INV16", "COMMUNICATION_STATUS")
   local inv10_Com = WR.read("SN:INV17", "COMMUNICATION_STATUS")
   ...
   WR.setProp("SN:INV17", "TOTAL_INV_ONLINE", totalInverters)
   ...
   WR.setProp("SN:INV13", "TOTAL_INV_ONLINE", totalInverters)
   WR.setProp("SN:INV14", "TOTAL_INV_ONLINE", totalInverters)
   WR.setProp("SN:INV15", "TOTAL_INV_ONLINE", totalInverters)
   WR.setProp("SN:INV16", "TOTAL_INV_ONLINE", totalInverters)
