...
function logCsv(file, msg)
 ...
 if file1 == nil then
  ...
  file1:write(anlagen_id..",SN:DG_LOG_IM01_,DG_LOG,0.0.0.0,3".."\n")
 ...
 if file2 == nil then
  fileNameZE = filePath.."/"..anlagen_id.."_ZE_LOG_IM01_"..string.sub(now, 1, 10).."_3.csv"
  file = fileNameZE
  file2 = io.open(file,"a")
  file2:write(anlagen_id..",SN:ZE_LOG_IM01_,ZE_LOG,0.0.0.0,5".."\n")
...
if not(settings.INVERTER[devS].dcCapacity and settings.INVERTER[devS].prRealRadSetpoint and settings.INVERTER[devS].prdcRealRadSetpoint and settings.INVERTER[devS].Module_inseries and settings.INVERTER[devS].Module_voc) then
 ...
 totalInvertersAcCapacity = settings.PLANT.totalInvertersAcCapacityS1 or 3
...
if (dev == lastDev) then
 if ampZEFUHF == nil then
  ...
  function(dev)
   ...
   --[[for devV in pairs(dgCtlDev) do
     local qacmode = WR.read(devV, "QAC_MODE")
     local qacmodeWrite = "QAC_MODE"
           local qacramp = WR.read(devV, "QAC_RMPTMS")
     local qacrampWrite = "QAC_RMPTMS"
           local qactimeout = WR.read(devV, "QAC_RVRTTMS")
     local qactimeoutWrite = "QAC_RVRTTMS"
     ...
     if  ((qacmode ~= 2) or (qacramp ~= 5) or (qactimeout ~= 3600) or (pacramp ~= 5) or (pactimeout ~= 90) or (cmdEnable ~= 1) or (pacwintime ~= 0)) then
      if WR.isOnline(devV) then
       WR.writeHexOpts(devV, qacmodeWrite, bit.tohex(2,4),0x6)
       WR.writeHexOpts(devV, pacrampWrite, bit.tohex(5,4),0x6)
             WR.writeHexOpts(devV, qacrampWrite, bit.tohex(5,4),0x6)
             WR.writeHexOpts(devV, qactimeoutWrite, bit.tohex(3600,4),0x6)
   ...
   for devV in pairs(dgCtlDev) do
    ...
    if not(is_nan(invPac)) then pvPac = pvPac + invPac end
        ...
        local pvQac = 0
   for devV in pairs(dgCtlDev) do
    local invQac = WR.read(devV, "QAC") * (1)
    if not(is_nan(invQac)) then pvQac = pvQac + invQac end
   end

   local zeQac = WR.read(dev, "ZE_QAC")
   --if is_nan(zeQac) then zeQac = 0 end

   local qmodeEnable = WR.read(dev, "QMODE_ENABLE")
   local maxQacLimit = WR.read(dev, "QAC_KVAR_LIMIT") * (1)
   if is_nan(maxQacLimit) then maxQacLimit = 0 end
   local minQacLimit = maxQacLimit
   for devV in pairs(dgCtlDev) do
    local invQacLimit = WR.read(dev, "QAC_KVAR_LIMIT") * (1)
   ...
   local qmodeEnableWrite = "QMODE_ENABLE"
   local gridConnWrite = "GRID_CONNECT"
--]]--
   local inv1_Com = WR.read("SN:INV01", "COMMUNICATION_STATUS")
   local inv2_Com = WR.read("SN:INV02", "COMMUNICATION_STATUS")  
   local inv3_Com = WR.read("SN:INV03", "COMMUNICATION_STATUS")
   local inv4_Com = WR.read("SN:INV04", "COMMUNICATION_STATUS")
   local inv5_Com = WR.read("SN:INV05", "COMMUNICATION_STATUS")
   local inv6_Com = WR.read("SN:INV06", "COMMUNICATION_STATUS")
   local inv7_Com = WR.read("SN:INV07", "COMMUNICATION_STATUS")
   local inv8_Com = WR.read("SN:INV08", "COMMUNICATION_STATUS")
   local inv9_Com = WR.read("SN:INV09", "COMMUNICATION_STATUS")
   local inv10_Com = WR.read("SN:INV10", "COMMUNICATION_STATUS")
   ...
   if (inv6_Com < 1) then totalInverters = totalInverters + 1 end
   if (inv7_Com < 1) then totalInverters = totalInverters + 1 end
   if (inv8_Com < 1) then totalInverters = totalInverters + 1 end
   if (inv9_Com < 1) then totalInverters = totalInverters + 1 end
   if (inv10_Com < 1) then totalInverters = totalInverters + 1 end
   
   WR.setProp("SN:INV01", "TOTAL_INV_ONLINE", totalInverters)
   WR.setProp("SN:INV02", "TOTAL_INV_ONLINE", totalInverters)
   WR.setProp("SN:INV03", "TOTAL_INV_ONLINE", totalInverters)
