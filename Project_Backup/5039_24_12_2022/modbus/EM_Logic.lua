...
if devS=="EM01" then
 ...
 AVG(deviceIn1, deviceIn2, deviceIn3, deviceIn4, deviceOut, "PR",            "TOTAL_PR")
 AVG(deviceIn1, deviceIn2, deviceIn3, deviceIn4, deviceOut, "PR_DAY",        "TOTAL_PR_DAY")
end
...
local pac_T = WR.read(dev, "TOTAL_PAC")
if (pac_T > 1) then
...
elseif ((pac_T <= 1) and  (startTime[dev].ts ~= 0) and ((stopTime[dev].againStart == 1) or (stopTime[dev].ts == 0))) then
...
local paclimit3 = WR.read(dev, "PAC_LIMIT3")
local paclimit4 = WR.read(dev, "PAC_LIMIT4")

if ((radiation > 25) and ((paclimit1 < 100) or (paclimit2 < 100)) or ((paclimit4 < 100) or (paclimit3 < 100))) then
